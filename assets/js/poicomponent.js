...
import POITweets from './poitweets'
	...
	header() {
		...
		return <div>
			<ul className="bv-tab-list-style-2">
				...
				<li>
					<Link
						to={`/poi/${match.params.poiname}/tweets`}
						className={match.params.type === 'tweets' ? 'active' : ''}
					>
						Tweets
					</Link>
				</li>
				<li>
					<Link
						to={`/poi/${match.params.poiname}/articles`}
...
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(withRouter(POIComponent))
